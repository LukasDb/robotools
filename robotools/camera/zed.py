...
class ZedCamera(Camera):
    ...
    def __init__(self, serial_number: str = '9999') -> None:
        ...
        self.init_params.coordinate_units = sl.UNIT.METER  # Use millimeter units (for depth measurements)
    ...
    def get_frame(self) -> CamFrame:
        ...
        if self.device.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            output.depth = np.nan_to_num(output.depth, copy=False, nan=0, posinf=0, neginf=0) # converting erroneous pixels to zero, to comply with the realsense standard
            ...
            output.depth_R = np.nan_to_num(output.depth_R, copy=False, nan=0, posinf=0, neginf=0) # converting erroneous pixels to zero, to comply with the realsense standard
